...
namespace Imgur.API.JsonConverters
{
    ...
    public class NotificationConverter : JsonConverter
    {
        ...
        {
            if (typeof(IMessageNotification).GetType().IsAssignableFrom(objectType.GetType()))
                return true;

            return typeof(ICommentNotification).GetType().IsAssignableFrom(objectType.GetType());
        }
