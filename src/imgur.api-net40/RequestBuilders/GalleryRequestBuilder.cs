...
namespace Imgur.API.RequestBuilders
{
    internal class GalleryRequestBuilder : RequestBuilderBase
    {
            ...
            ImageFileType? fileType = null, ImageSize? imageSize = null)
        {
            ...
            if (!string.IsNullOrWhiteSpace(qAll))
                query.Append($"&q_all={HttpUtility.UrlEncode(qAll)}");

            if (!string.IsNullOrWhiteSpace(qAny))
                query.Append($"&q_any={HttpUtility.UrlEncode(qAny)}");

            if (!string.IsNullOrWhiteSpace(qExactly))
                query.Append($"&q_exactly={HttpUtility.UrlEncode(qExactly)}");

            if (!string.IsNullOrWhiteSpace(qNot))
                query.Append($"&q_not={HttpUtility.UrlEncode(qNot)}");

            if (fileType != null)
                query.Append($"&q_type={HttpUtility.UrlEncode(fileType.ToString().ToLower())}");

            if (imageSize != null)
                query.Append($"&q_size_px={HttpUtility.UrlEncode(imageSize.ToString().ToLower())}");
        ...
        internal static string SearchGalleryRequest(string url, string query)
        {
            ...
            return $"{url}?q={HttpUtility.UrlEncode(query)}";
        }
    }
}